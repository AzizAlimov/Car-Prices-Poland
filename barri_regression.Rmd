...
{ set.seed(123)
  ...
  { indices = (((k-1)*nhold+1):(k*nhold))
    ...
    pred1 = predict(models[[k]],newdata=z,interval="prediction",level=a)
    pred = exp(pred1)
    res1 = trainpred
    RSS = c(crossprod(models[[k]]$residuals))
    MSE = RSS / length(models[[k]]$residuals)
...
wTrain=w[trainIndex]
wHoldo=w[-trainIndex] 
WLSTrain=lm(price~mark_cat+year+mileage_cat+log_vol_engine+fuel+car_type+gdpc,data=train, weights = 1/(year)^2)
plot(WLSTrain$fitted.values, WLSTrain$residuals)
plot(train$year, model_normal_variables$residuals, ylab="Residuals",xlab="Year")
```

```{r}
modelc_pred_5=predict(WLSTrain,newdata=holdout,interval="prediction",level=0.5, weights = 1/(holdout$year)^2)
modelc_pred_8=predict(WLSTrain,newdata=holdout,interval="prediction",level=0.8, weights = 1/(holdout$year)^2)
ISOLS50=intervalScore(modelc_pred_5,holdout$price,0.5)
ISOLS80=intervalScore(modelc_pred_8,holdout$price,0.8)
out_modelc=rbind(ISOLS50$summary,ISOLS80$summary)
colnames(out_modelc)=c("level","avgleng","IS","cover")
