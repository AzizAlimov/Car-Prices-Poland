...
{ set.seed(123)
  ...
  { indices = (((k-1)*nhold+1):(k*nhold))
    ...
    train = df[-indices,]
    holdout = df[indices,]
    obj = lm(price~mark_cat+year+mileage_cat+log_vol_engine+fuel+car_type+gdpc,data=df)
    pred = predict(obj,newdata=holdout,interval="prediction",level=a)
    RSS = c(crossprod(obj$residuals))
    MSE = RSS / length(obj$residuals)
    x = intervalScore(pred, holdout$price, a)
    y= sqrt(MSE)
    z = summary(obj)$r.squared
...
```{r}
modelc_pred_5=predict(model_normal_variables,newdata=holdout,interval="prediction",level=0.5)
modelc_pred_8=predict(model_normal_variables,newdata=holdout,interval="prediction",level=0.8)
ISOLS50=intervalScore(modelc_pred_5,holdout$price,0.5)
ISOLS80=intervalScore(modelc_pred_8,holdout$price,0.8)
out_modelc=rbind(ISOLS50$summary,ISOLS80$summary)
colnames(out_modelc)=c("level","avgleng","IS","cover")
...
```{r}
modelc_pred_5=predict(model_transform_variables,newdata=holdout,interval="prediction",level=0.5)
modelc_pred_8=predict(model_transform_variables,newdata=holdout,interval="prediction",level=0.8)
ISOLS50=intervalScore(modelc_pred_5,holdout$price,0.5)
ISOLS80=intervalScore(modelc_pred_8,holdout$price,0.8)
out_modelc=rbind(ISOLS50$summary,ISOLS80$summary)
colnames(out_modelc)=c("level","avgleng","IS","cover")
...
```{r}
modelc_pred_5=predict(model_original_variables,newdata=holdout,interval="prediction",level=0.5)
modelc_pred_8=predict(model_original_variables,newdata=holdout,interval="prediction",level=0.8)
ISOLS50=intervalScore(modelc_pred_5,holdout$price,0.5)
ISOLS80=intervalScore(modelc_pred_8,holdout$price,0.8)
out_modelc=rbind(ISOLS50$summary,ISOLS80$summary)
colnames(out_modelc)=c("level","avgleng","IS","cover")
...
```{r}
modelc_pred_5=predict(model3,newdata=holdout,interval="prediction",level=0.5)
modelc_pred_8=predict(model3,newdata=holdout,interval="prediction",level=0.8)
ISOLS50=intervalScore(modelc_pred_5,holdout$price,0.5)
ISOLS80=intervalScore(modelc_pred_8,holdout$price,0.8)
out_modelc=rbind(ISOLS50$summary,ISOLS80$summary)
colnames(out_modelc)=c("level","avgleng","IS","cover")
...
```{r}
SD1 = tapply(model_chosen_variables$resid,train$car_type, sd)
print(SD1)
SD2 = tapply(model_chosen_variables$resid,train$fuel, sd)
print(SD2)
n=nrow(subset_trial)
```
...
```{r}
sqrtz=rep(1,n) 
sqrtz[subset_trial$mark_cat=="Euro"]=0.1
sqrtz[subset_trial$mark_cat=="US"]=0.1
sqrtz[subset_trial$mark_cat=="Asia"]=0.1
z=sqrtz^2
w=1/z # weight functions for WLS
wTrain=w[trainIndex]
wHoldo=w[-trainIndex] 
WLSTrain=lm(price~mark_cat+year+mileage_cat+log_vol_engine+fuel+car_type+gdpc,data=train, weights = sqrt(mileage)/sqrt(year))
plot(WLSTrain$fitted.values, WLSTrain$residuals)
plot(train$price, model_normal_variables$residuals)
```


```{r}
modelc_pred_5=predict(WLSTrain,newdata=holdout,interval="prediction",level=0.5, weights = wHoldo)
modelc_pred_8=predict(WLSTrain,newdata=holdout,interval="prediction",level=0.8, weights = wHoldo)
ISOLS50=intervalScore(modelc_pred_5,holdout$price,0.5)
ISOLS80=intervalScore(modelc_pred_8,holdout$price,0.8)
out_modelc=rbind(ISOLS50$summary,ISOLS80$summary)
colnames(out_modelc)=c("level","avgleng","IS","cover")
